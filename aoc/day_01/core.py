...
def parse_input(raw_input: str) -> Tuple[Tuple[int], Tuple[int]]:
...
def calc_distances(l1: Tuple[int], l2: Tuple[int]) -> int:
    # Sum the absolute value of each l1 - l2
    return sum(abs(loc_1 - loc_2) for loc_1, loc_2 in zip(sorted(l1), sorted(l2)))


def calc_similarity_score(l1: Tuple[int], l2: Tuple[int]) -> int:
    # Multiply each number in l1 with the number of times it appears in l2
    counts = Counter(l2)
    return sum(loc * counts[loc] for loc in l1)
...
def part_1(puzzle_input: str = p1) -> int:
    ...
    another 3. The distance between them is 1. The third-smallest number in
    both lists is 3, so the distance between them is 0. The next numbers to
    pair up are 3 and 4, a distance of 1. The fifth-smallest numbers in each
    list are 3 and 5, a distance of 2. Finally, the largest number in the left
    list is 4, while the largest number in the right list is 9; these are a
    distance 5 apart. To find the total distance between the left list and the
...
def part_2(puzzle_input: str = p1) -> int:
    ...
    right list. The fifth number, 3, appears in the right list three times; the
    similarity score increases by 9. The last number, 3, appears in the right
    list three times; the similarity score again increases by 9.
    ...
    """
    l1, l2 = parse_input(puzzle_input)
    answer = calc_similarity_score(l1, l2)
    # breakpoint()
    return answer
