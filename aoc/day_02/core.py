...
def is_decreasing(report: Report) -> bool:
    return list(report) == sorted(report, reverse=True)


def is_increasing(report: Report) -> bool:
    return list(report) == sorted(report)
...
def is_safe_p2(report: Report) -> bool:
    if is_safe(report):
        return True
    # Try safety when removing any item from report
    for idx, _ in enumerate(report):
        t_report = list(report)
        t_report.pop(idx)
        if is_safe(t_report):
            return True
    return False
...
def part_1(puzzle_input: str = p1) -> int:
    ...
    reports = util.parse_num_matrix(puzzle_input)
    safe_reports = [report for report in reports if is_safe(report)]
    return len(safe_reports)


def part_2(puzzle_input: str = p1) -> int:
    """How many reports are safe if there can be one fault per report"""
    reports = util.parse_num_matrix(puzzle_input)
    safe_reports = [report for report in reports if is_safe_p2(report)]
    # breakpoint()
    return len(safe_reports)
